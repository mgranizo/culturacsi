	...
	if ( function_exists( 'culturacsi_portal_render_process_tutorial' ) ) {
		echo culturacsi_portal_render_process_tutorial(
			array(
				...
				'open'    => false,
				'steps'   => array(
					array( 'text' => 'Usa la ricerca per trovare la notizia da gestire.' ),
					array( 'text' => 'Apri Mod. per modificare il contenuto.' ),
					array( 'text' => 'Usa Appr./Rif. per la moderazione (solo Site Admin).' ),
				),
			)
		); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
	echo '<div class="assoc-portal-events-list assoc-portal-section">';
	$export_url = function_exists( 'culturacsi_export_build_url' ) ? culturacsi_export_build_url( 'news', (string) ( $_SERVER['REQUEST_URI'] ?? '' ) ) : (string) add_query_arg( 'culturacsi_export', 'news', $_SERVER['REQUEST_URI'] ?? '' );
