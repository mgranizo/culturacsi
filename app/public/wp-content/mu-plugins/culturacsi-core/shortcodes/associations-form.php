...
/**
 * Shortcode handler for the Association creation and editing form.
 * Provides a frontend interface for users to manage associations.
 */
function culturacsi_portal_associations_form_shortcode(): string {
	...
	// Non-admins are routed to a different, potentially more restricted shortcode
	if ( ! current_user_can( 'manage_options' ) ) {
		return culturacsi_portal_association_form_shortcode();
	}

	// 2. Determine if we are editing an existing association
	...
	// Validate that if an ID was provided, it actually fetched a valid association post
	...
	// Helpers for sanitizing and normalizing comma-separated multi-value text fields (e.g. phone numbers)
	...
	// Helper for sanitizing and normalizing multi-value email fields
	...
	if ( 'POST' === strtoupper( (string) $_SERVER['REQUEST_METHOD'] ) && ( isset( $_POST['culturacsi_associations_form_submit'] ) || isset( $_REQUEST['is_portal_ajax'] ) ) ) {
		...
		// 3a. Verify Nonce for CSRF protection
		if ( ! isset( $_POST['culturacsi_associations_form_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['culturacsi_associations_form_nonce'] ) ), 'culturacsi_associations_form_save' ) ) {
			...
			// 3b. Context validation (are we saving an existing or new item?)
			...
			if ( $form_association_id > 0 && ( ! $form_association instanceof WP_Post || 'association' !== $form_association->post_type ) ) {
				...
				// 3c. Field mapping and sanitization
				...
				// Author validation fallback
				$author_user = get_user_by( 'id', $author_id );
				if ( $author_id <= 0 || ! ( $author_user instanceof WP_User ) ) {
					$author_id = get_current_user_id();
				}

				// 3d. Database Insertion / Updating if no prior validation errors
				if ( '' === $message_html ) {
					$post_data = array(
					...
					if ( is_wp_error( $saved_id ) ) {
						...
						foreach ( $meta_fields as $key ) {
							...
							// Specific normalization for multi-value / specific formats
							if ( 'phone' === $key ) {
						...
						// Mirror 'region' to 'regione' for backward compatibility
						update_post_meta( $saved_id, 'regione', (string) get_post_meta( $saved_id, 'region', true ) );

						// Save taxonomies (activity_category)
						$term_ids = array();
						...
						// Process Featued Image (Logo) uploads
						...
						// 3e. AJAX vs Standard response handling
						...
						// Standard Redirect on save success
	...
	// Read save success flag from URL for displaying success notice
	...
	// Inject UX Guidance instructions automatically if the module is active
	...
	<form class="assoc-portal-form" method="post" enctype="multipart/form-data" data-redirect-url="<?php echo esc_url( $root_url ); ?>">
		...
		<p><?php echo culturacsi_portal_label_with_tip( 'post_title', 'Nome associazione *', 'Inserisci il nome completo ufficiale.' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			<input type="text" id="post_title" name="post_title" required value="<?php echo esc_attr( $association instanceof WP_Post ? (string) $association->post_title : '' ); ?>">
		</p>
		...
		<p><label for="city">Città</label><input type="text" id="city" name="city" value="<?php echo esc_attr( (string) get_post_meta( $association_id, 'city', true ) ); ?>"></p>
		...
		<p><label for="phone">Telefono</label><textarea id="phone" name="phone" rows="2" placeholder="Es. +39 333 1234567, 02 1234567"><?php echo esc_textarea( (string) get_post_meta( $association_id, 'phone', true ) ); ?></textarea><small>Puoi inserire più numeri separati da virgola o una per riga.</small></p>
		<p><label for="email">Email</label><textarea id="email" name="email" rows="2" placeholder="Es. info@dominio.it, segreteria@dominio.it"><?php echo esc_textarea( (string) get_post_meta( $association_id, 'email', true ) ); ?></textarea><small>Puoi inserire più email separate da virgola o una per riga.</small></p>
		...
		<fieldset>
			<legend>Categorie Attività</legend>
			<div class="category-checklist">
				<?php
				// Output custom activity category checklist if available, otherwise fallback to native checklist
