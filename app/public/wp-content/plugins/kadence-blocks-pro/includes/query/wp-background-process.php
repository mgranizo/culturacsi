...
abstract class KadenceWP_Kadence_Blocks_Pro_WP_Background_Process extends KadenceWP_Kadence_Blocks_Pro_WP_Async_Request {
	...
	public function get_batches( $limit = 0 ) {
		...
		$items = $wpdb->get_results( $wpdb->prepare( $sql, $args ) ); //phpcs:ignore
	...
	public function schedule_cron_healthcheck( $schedules ) {
		...
		if ( 1 === $interval ) {
			$display = __( 'Every Minute', 'kadence-blocks-pro' );
		} else {
			// translators: %d: minutes interval.
			$display = sprintf( __( 'Every %d Minutes', 'kadence-blocks-pro' ), $interval );
		}
