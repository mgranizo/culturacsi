<?php
/**
 * Handles CSV export. This is a near copy from the woocommerce csv exporter.
 *
...
/**
 * KB_CSV_Exporter Class.
 */
abstract class KB_CSV_Exporter {
	 ...
	 * @var string
	 */
	protected $filename = 'kb-export.csv';
	...
	public function get_column_names() {
		return apply_filters( "kadence_blocks_{$this->export_type}_export_column_names", $this->column_names, $this );
	}
	...
	public function set_column_names( $column_names ) {
		...
		foreach ( $column_names as $column_id => $column_name ) {
			$this->column_names[ sanitize_text_field( $column_id ) ] = sanitize_text_field( $column_name );
		}
	}
	...
	public function get_delimiter() {
		return apply_filters( "kadence_blocks_{$this->export_type}_export_delimiter", $this->delimiter );
	}
	...
	public function set_columns_to_export( $columns ) {
		$this->columns_to_export = array_map( 'kadence_blocks_pro_clean', $columns );
	}
	...
	public function send_headers() {
		...
		kadence_blocks_pro_set_time_limit( 0 );
		nocache_headers();
	...
	public function get_filename() {
		return sanitize_file_name( apply_filters( "kadence_blocks_{$this->export_type}_export_get_filename", $this->filename ) );
	}
	...
	protected function export_rows() {
		...
		return apply_filters( "kadence_blocks_{$this->export_type}_export_rows", ob_get_clean(), $this );
	}
	...
	public function get_limit() {
		return apply_filters( "kadence_blocks_{$this->export_type}_export_batch_limit", $this->limit, $this );
	}
	...
	public function format_data( $data ) {
		...
		if ( $use_mb ) {
			...
			$data     = 'UTF-8' === $encoding ? $data : utf8_encode( $data );//phpcs:ignore Generic.PHP.DeprecatedFunctions.Deprecated
		}
