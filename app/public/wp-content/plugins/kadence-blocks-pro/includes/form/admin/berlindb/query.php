...
class Query extends Base {
	...
	private function delete_all_item_meta( $item_id = 0 ) {
		...
		$item_table = $this->apply_prefix( $this->item_name );
		if ( $item_table === 'kbp_entry' ) {
			$item_table = 'kbp_form_entry';
		}

		// Delete all meta data for this item ID
		foreach ( $meta_ids as $mid ) {
			delete_metadata_by_mid( $item_table, $mid );
		}
	}
