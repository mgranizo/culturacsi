...
function header_account() {
	if ( ! is_user_logged_in() || ( is_customize_preview() && 'out' === kadence()->option( 'header_account_preview' ) ) ) {
		...
		if ( 'link' === $action ) {
			echo '<a href="' . ( kadence()->option( 'header_account_link' ) ? esc_url( kadence()->option( 'header_account_link' ) ) : '#' ) . '"' . ( ! empty( $label ) ? ' aria-label="' . esc_attr__( $label, 'kadence-pro' ) . '"' : ' aria-label="' . esc_attr__( 'Account', 'kadence-pro' ) . '"' ) . ' class="header-account-button">';
			switch ( $style ) {
				case 'icon':
			...
			echo '<button data-toggle-target="#login-drawer"' . ( ! empty( $label ) ? ' aria-label="' . esc_attr__( $label, 'kadence-pro' ) . '"' : ' aria-label="' . esc_attr__( 'Login', 'kadence-pro' ) . '"' ) . ' class="drawer-toggle header-account-button" data-toggle-body-class="showing-popup-drawer" aria-expanded="false" data-set-focus=".login-toggle-close">';
			switch ( $style ) {
				case 'icon':
					kadence()->print_icon( $icon, '', false );
					break;
...
function header_mobile_account() {
	if ( ! is_user_logged_in() || ( is_customize_preview() && 'out' === kadence()->option( 'header_mobile_account_preview' ) ) ) {
		...
		if ( 'link' === $action ) {
			echo '<a href="' . ( kadence()->option( 'header_mobile_account_link' ) ? esc_url( kadence()->option( 'header_mobile_account_link' ) ) : '#' ) . '"' . ( ! empty( $label ) ? ' aria-label="' . esc_attr__( $label, 'kadence-pro' ) . '"' : ' aria-label="' . esc_attr__( 'Account', 'kadence-pro' ) . '"' ) . ' class="header-account-button">';
			switch ( $style ) {
				case 'icon':
					kadence()->print_icon( $icon, '', false );
					break;
