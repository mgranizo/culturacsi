...
class Elements_Controller {
	...
	public function __construct() {
		...
		add_action( 'admin_bar_menu', array( $this, 'active_elements_admin_bar' ), 100 );
		add_action( 'admin_bar_menu', array( $this, 'add_new_element_to_admin_bar' ), 100 );
	}
	/**
	...
	public function active_elements_admin_bar( $wp_admin_bar ) {
		if ( empty( self::$active_elements ) || ! current_user_can( 'edit_posts' ) ) {
			return;
		}

		$wp_admin_bar->add_node( array(
			'id'    => 'kadence-active-elements',
			//translators: %s is the number of active elements
			'title' => sprintf( __( 'Elements (%s)', 'kadence-pro' ), count( self::$active_elements ) ),
			'href'  => admin_url( 'edit.php?post_type=kadence_element' ),
		) );

		foreach ( self::$active_elements as $element_id => $meta ) {
			...
			$wp_admin_bar->add_node( array(
				'id'     => 'kadence-element-' . $element_id,
				'title'  => $post->post_title,
				'parent' => 'kadence-active-elements',
				'href'   => get_edit_post_link( $element_id ),
			) );
		}
		$wp_admin_bar->add_node( array(
			'id'     => 'kadence-element-add-new',
			'title'  => __( 'Add New', 'kadence-pro' ),
			'parent' => 'kadence-active-elements',
			'href'   => admin_url( 'post-new.php?post_type=kadence_element' ),
		) );
	}

	/**
	 * Adds a "Kadence Element" link to the WordPress admin bar "New" dropdown menu.
	 *
	 * @param WP_Admin_Bar $wp_admin_bar The WP_Admin_Bar instance.
	 */
	public function add_new_element_to_admin_bar( $wp_admin_bar ) {
		// Only show for users who can create elements
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			return;
		}

		$wp_admin_bar->add_node( array(
			'id'     => 'new-kadence-element',
			'title'  => __( 'Element', 'kadence-pro' ),
			'parent' => 'new-content',
			'href'   => admin_url( 'post-new.php?post_type=kadence_element' ),
		) );
	}
